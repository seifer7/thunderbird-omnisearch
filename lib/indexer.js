...
(function () {
  ...
  async function getIndexStartDate() {
    try {
      ...
      // Parse as local midnight to match <input type="date"> semantics.
      const t = new Date(`${s.indexStartDate}T00:00:00`).getTime();
  ...
  async function fullBuild(engine, extractor, onProgress) {
    ...
    const total = Math.max(
      startDateMs ? await countEligibleHeaders(folders, startDateMs) : await totalMessageCount(folders),
      1
    );
