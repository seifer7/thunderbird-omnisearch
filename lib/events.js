...
(function () {
  ...
  async function expandedExcludedFolderIds(folderFilter) {
    ...
    const expanded = new Set(folderFilter.excludedFolderIds);
    try {
      const accounts = await messenger.accounts.list(true);
      for (const account of accounts) {
        const stack = (account.folders || (account.rootFolder && account.rootFolder.subFolders) || []).map(
          (folder) => ({ folder, inExcludedTree: false }),
        );
        while (stack.length) {
          const { folder, inExcludedTree } = stack.pop();
          const excludedHere = inExcludedTree || folderFilter.excludedFolderIds.has(folder.id);
          if (excludedHere) expanded.add(folder.id);
