...
(function () {
  async function upsertHeader(ctrl, header, excluded) {
    // Don't index messages from accounts the user excluded in settings.
    if (excluded && header.folder && excluded.has(header.folder.accountId)) return;
  ...
  function registerEvents(ctrl) {
    ...
    on(m.onNewMailReceived, async (_folder, messageList) => {
      const excluded = await OmniIndexer.getExcludedAccounts();
      for (const header of headersOf(messageList)) await upsertHeader(ctrl, header, excluded);
      ctrl.persistSoon();
    });

    on(m.onUpdated, async (message) => {
      const excluded = await OmniIndexer.getExcludedAccounts();
      await upsertHeader(ctrl, message, excluded);
      ctrl.persistSoon();
    });

    on(m.onDeleted, (messageList) => {
      // Removal is always safe regardless of account inclusion.
    ...
    on(m.onMoved, async (originalMessages, movedMessages) => {
      ...
      for (const header of headersOf(movedMessages)) await upsertHeader(ctrl, header, excluded);
      ctrl.persistSoon();
    });

    on(m.onCopied, async (_originalMessages, copiedMessages) => {
      const excluded = await OmniIndexer.getExcludedAccounts();
      for (const header of headersOf(copiedMessages)) await upsertHeader(ctrl, header, excluded);
      ctrl.persistSoon();
    });
  }
