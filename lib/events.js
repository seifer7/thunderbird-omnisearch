...
(function () {
  ...
  async function expandedExcludedFolderIds(folderFilter) {
    ...
    try {
      ...
      const walk = (folders, inExcludedTree) => {
        for (const folder of folders || []) {
          ...
          walk(folder.subFolders, excludedHere);
        }
      };
      for (const account of accounts) {
        walk(account.folders || (account.rootFolder && account.rootFolder.subFolders), false);
      }
    } catch (e) {
      // Fall back to explicit ids only.
    }
    return expanded;
  }
