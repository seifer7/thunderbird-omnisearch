...
(function () {
  let expandedExcludedCacheKey = null;
  let expandedExcludedCacheValue = null;
  ...
  async function upsertHeader(ctrl, header, opts) {
    ...
    if (folder) {
      ...
      if (opts.folderFilter) {
        ...
        if (ff.mode === 'all' && ff.expandedExcludedFolderIds.has(folder.id)) return;
      }
    }
  ...
  function accountTopFolders(account) {
    if (account.folders) return account.folders;
    if (account.rootFolder && account.rootFolder.subFolders) return account.rootFolder.subFolders;
    return [];
  }

  function excludedFolderCacheKey(folderFilter) {
    if (!folderFilter || folderFilter.mode !== 'all') return null;
    if (
      !folderFilter.excludedFolderIds ||
      typeof folderFilter.excludedFolderIds[Symbol.iterator] !== 'function'
    ) {
      return null;
    }
    return `all:${[...folderFilter.excludedFolderIds].sort().join(',')}`;
  }

  // In 'all' mode, excluded folders represent whole subtrees. Expand the explicit
  // excluded ids to include descendants so live event filtering matches the full
  // folder walk used by builds/reconcile.
  async function expandedExcludedFolderIds(folderFilter) {
    if (
      !folderFilter ||
      folderFilter.mode !== 'all' ||
      !folderFilter.excludedFolderIds ||
      !folderFilter.excludedFolderIds.size
    ) {
      return folderFilter && folderFilter.excludedFolderIds;
    }
    const cacheKey = excludedFolderCacheKey(folderFilter);
    if (cacheKey && expandedExcludedCacheKey === cacheKey && expandedExcludedCacheValue) {
      return expandedExcludedCacheValue;
    }
    const excludedIds = folderFilter.excludedFolderIds;
    let expanded = excludedIds;
    try {
      const accounts = await messenger.accounts.list(true);
      for (const account of accounts) {
        const stack = accountTopFolders(account).map((folder) => ({ folder, inExcludedTree: false }));
        while (stack.length) {
          const { folder, inExcludedTree } = stack.pop();
          const explicit = excludedIds.has(folder.id);
          const excludedHere = inExcludedTree || explicit;
          if (excludedHere && !explicit) {
            if (expanded === excludedIds) expanded = new Set(excludedIds);
            expanded.add(folder.id);
          }
          for (const child of folder.subFolders || []) {
            stack.push({ folder: child, inExcludedTree: excludedHere });
          }
        }
      }
      expandedExcludedCacheKey = cacheKey;
      expandedExcludedCacheValue = expanded;
    } catch (e) {
      console.warn(
        `[OmniSearch] failed to expand excluded folder descendants (key: ${cacheKey || 'none'}); falling back to explicit exclusions only:`,
        e,
      );
    }
    return expanded;
  ...
  async function indexOpts() {
    const folderFilter = await OmniIndexer.getFolderFilter();
    folderFilter.expandedExcludedFolderIds = await expandedExcludedFolderIds(folderFilter);
    return {
