...
(function () {
  const OPERATORS = ['from', 'to', 'after', 'before', 'date', 'newer_than', 'older_than'];

  // Named periods. Values, not operators, so they compose with the period rule
  // for free: `after:today`, `before:yesterday`, `date:yesterday..today`.
  // this-week starts on MONDAY (ISO). That is a real choice — some locales start
  // the week on Sunday — so it is pinned by a test rather than left to chance.
  const PRESETS = ['today', 'yesterday', 'this-week', 'this-month', 'this-year'];

  // Gmail-compatible relative durations. Units step the CALENDAR for months and
  // years, so "3m" respects month lengths instead of assuming 30 days.
  const DURATION_RE = /^(\d{1,4})([dwmy])$/;
  ...
  // A whole day, expressed as a period like any other.
  const dayPeriod = (date) => {
    const start = midnight(date);
    const next = new Date(start);
    next.setDate(next.getDate() + 1);
    return { start: start.getTime(), end: next.getTime() - 1 };
  };

  // Resolve a named period against `now`.
  function presetPeriod(name, now) {
    const today = midnight(now);
    if (name === 'today') return dayPeriod(today);
    if (name === 'yesterday') {
    ...
    if (name === 'this-week') {
      ...
      // getDay(): 0 = Sunday. Shift back to the preceding Monday.
    ...
    if (name === 'this-month') {
      return period(today.getFullYear(), today.getMonth() + 1, null);
    }
    return period(today.getFullYear(), null, null); // this-year
  }

  // The instant a relative duration points at, snapped to a day boundary so the
  // window does not slide as the clock ticks and results stay stable while the
  // user reads them.
  function durationStart(count, unit, now) {
  ...
  function parsePeriod(rawValue, now) {
    ...
    if (PRESETS.includes(value)) return OK(presetPeriod(value, now));
  ...
  function resolveDateOperator(op, rawValue, now) {
    if (op === 'newer_than' || op === 'older_than') {
      const raw = String(rawValue || '').trim();
      if (!raw) return INCOMPLETE; // "newer_than:" mid-typing
      const m = DURATION_RE.exec(raw.toLowerCase());
      if (!m) {
        return ERR(`Unrecognised time span "${raw}" — use a number and a unit, e.g. 7d, 2w, 3m, 1y.`);
      }
      const at = durationStart(+m[1], m[2], now);
      // Complementary by construction: newer_than starts AT the boundary,
      // older_than ends one millisecond before it, so every message falls in
      // exactly one of the two and neither overlaps the other.
      return op === 'newer_than' ? OK({ after: at, before: null }) : OK({ after: null, before: at - 1 });
    }

    const rangeParts = rawValue.split('..');
    if (rangeParts.length === 2) {
      if (!rangeParts[0] || !rangeParts[1]) return INCOMPLETE; // "2024-06.." mid-typing
      const a = parsePeriod(rangeParts[0], now);
      const b = parsePeriod(rangeParts[1], now);
    ...
    const p = parsePeriod(rawValue, now);
  ...
  function parse(query, now) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      for (let extra = maxExtra; extra >= 1; extra--) {
        ...
        const attempt = resolveDateOperator(op, candidate, at);
      ...
      if (!outcome) outcome = resolveDateOperator(op, rawValue, at);
