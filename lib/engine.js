...
(function () {
  ...
  class SearchEngine {
    ...
    // Async on purpose: the term-index rebuild is the multi-second cost when the
    // popup reopens after the background page was suspended. loadJSAsync yields to
    // the event loop every 1000 terms, so the UI never freezes. parsed.mini is
    // already the object shape loadJSAsync expects (it came from mini.toJSON() in
    // serialize()), so we hand it over directly — no wasteful re-stringify/re-parse.
    static async deserialize(json) {
      const parsed = JSON.parse(json);
      const engine = new SearchEngine();
      engine.mini = await MiniSearch.loadJSAsync(parsed.mini, MINISEARCH_OPTIONS);
