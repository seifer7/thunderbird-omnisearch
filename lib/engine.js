...
(function () {
  ...
  class SearchEngine {
    ...
    //
    // The deduplicated results are then re-scored by recency and re-sorted, and
    // only then truncated to `limit` — truncating first would return the top-N
    // by raw text score and merely reorder those, so a newer message just
    // outside that top-N could never surface. Every label copy of a message
    ...
    //
    // Returns { results, errors, filters, applied, text } rather than a bare array: a rejected
    // date MUST reach the user. Silently dropping it would run an unfiltered
    // search that looks like it worked, which is the exact failure the parser's
    // reject-don't-guess rule exists to prevent.
    //
    ...
    search(query, limit = 100, opts = {}) {
      ...
      const empty = { results: [], errors, filters, applied, text: q };
      ...
      return { results: out.slice(0, limit), errors, filters, applied, text: q };
    }
