...
importScripts('minisearch.js', 'query.js', 'dockey.js', 'engine.js', 'store.js');
...
// Cached ranking for the query currently being paged.
//
// A page-2 request MUST be served from the same ranked array that produced page
// 1. Re-ranking would re-run recencyFactor against a newer `now`, and the boost
// is a continuous function of age — enough drift to swap two adjacent results
// across the page boundary, which shows the user a duplicated message and hides
// another entirely. Caching the array makes a paged scroll a stable view of one
// ranking instead of a series of independent searches.
//
// One entry is enough: the popup only ever pages the query in its field. Every
// command that mutates the index drops it, so a page never comes from a ranking
// built against mail that has since changed.
let rankCache = null; // { query, list, meta }
...
async function persistNow() {
  ...
  try {
    await OmniStore.saveIndex(engine.toData(), { count: engine.size, headerOnly, updatedAt });
    saveError = null;
...
async function dispatch(cmd, msg) {
  switch (cmd) {
    case 'load': {
      ...
      if (data) {
        ...
        if (legacy && engine.size > 0) {
          console.log('[OmniSearch] migrating index to native format — next load will be fast…');
          void persistNow();
        }
        if (engine.migratedFromLegacyKey) {
          // Re-keyed onto stable identity. Persist now so the next load skips
          // the migration, and tell the main thread — only it can run the deep
          // sweep that repairs what the old keying let go missing.
          console.log('[OmniSearch] index re-keyed to stable ids —', engine.size, 'docs after collapsing label copies');
          if (!legacy) void persistNow();
        }
      }
      return { count: engine.size, headerOnly, updatedAt, saveError, rekeyed: !!engine.migratedFromLegacyKey };
    }
    case 'search': {
      // Returns { results, total, offset, hasMore, capped, errors, filters,
      // applied, text } — passed through whole so parse errors reach the UI.
      //
      // offset 0 always re-ranks: a fresh search must see the current index.
      // Only a genuine page-forward reuses the cache, and only for the same
      // query text.
      const query = msg.query || '';
      const offset = msg.offset || 0;
      if (offset === 0 || !rankCache || rankCache.query !== query) {
        const r = engine.rank(query);
        rankCache = { query, list: r.ranked, meta: r };
      }
      return OmniEngine.page(rankCache.list, msg.limit || OmniEngine.DEFAULT_LIMIT, offset, rankCache.meta);
    }
    case 'reset':
    ...
    case 'clear':
      ...
      saveError = null;
      return { count: 0, headerOnly, updatedAt, saveError };
    case 'addAll':
    ...
    case 'upsert':
      ...
      return { count: engine.size };
    case 'remove':
      for (const key of msg.ids || []) engine.remove(key);
      invalidateRankCache();
      persistSoon();
      return { count: engine.size };
    case 'knownIds':
    ...
    case 'hasKey':
      // One cheap round-trip the catch-up makes per candidate message, before
      // deciding whether to pay for reading and extracting the body.
      return { has: engine.hasKey(msg.key) };
    case 'maxDate':
      return { date: engine.maxDate(msg.accountId) };
    case 'removeFromFolder':
      // A message leaving one folder keeps the doc alive while it still lives in
      // another (Gmail/IMAP labels) — see lib/engine.js removeFromFolder.
      engine.removeFromFolder(msg.key, msg.folderName);
      invalidateRankCache();
      persistSoon();
      return { count: engine.size };
    case 'flush':
      ...
      await persistNow();
      return { count: engine.size, headerOnly, saveError };
    default:
      return {};
  }
}
